...
flash_latest:
	docker run --rm -v ${PWD}:/home/builder/esp_slip_router --device=$(filter-out $@,$(MAKECMDGOALS)) nulldevil/esp-open-sdk:latest sudo -- sh -c "export PATH=\"/home/builder/esp-open-sdk/xtensa-lx106-elf/bin:$$PATH\" && cd esp_slip_router && esptool.py --port $(filter-out $@,$(MAKECMDGOALS)) -fs 32m 0x00000 firmware/0x00000.bin 0x10000 firmware/0x10000.bin"
